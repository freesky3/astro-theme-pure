...
4. **Resting Potential ($V_{rest} = 0$)**: 灰色代表静息电位，一般设为0
...
**Ornstein-Uhlenbeck (OU) 过程：** 这个方程在数学上描述的是一个 **OU 过程**。在长时极限下，如果神经元没有发放阈值，电压 $V$ 的平稳分布将是一个均值为 $\mu$、方差为 $\frac{\sigma^2}{2}$ 的正态分布。这种参数化方式使得噪声强度 $\sigma$ 的物理意义更加明确，不随时间常数 $\tau$ 的改变而发生缩放冲突。
